...
| [Groq API](https://console.groq.com/) | LLM API for query expansion, reranking, and dataset generation | Pay-per-use | `GROQ_API_KEY` | [Quick Start Guide](https://console.groq.com/docs/quickstart) |
...
To authenticate to Groq's API, you must fill out the `GROQ_API_KEY` env var with an authentication token.

```env
GROQ_API_KEY=your_api_key_here
```

→ Check out this [tutorial](https://console.groq.com/docs/quickstart) to learn how to provide one from Groq.
...
make local-start
```
It should start much faster after the first run because it reuses the existing images and caches.

If you change code or dependencies and want to rebuild the images, run:
```shell
make local-build
```

Behind the scenes `make local-start` runs the Docker Compose stack defined in the [docker-compose.yml](https://github.com/decodingml/llm-twin-course/blob/main/docker-compose.yml) file.
...
Go to your [Hugging Face account](https://huggingface.co/), and under the **Models** section, you will see your `{your_name}/LLMTwin-Llama-3.1-8B` model. To run the evaluation and inference using your fine-tuned LLM, add your Hugging Face model ID to your `.env` file, such as `MODEL_ID=your-username/LLMTwin-Llama-3.1-8B`.
