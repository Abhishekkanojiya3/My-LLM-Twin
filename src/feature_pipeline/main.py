...
from data_flow.stream_input import RedisSource
...
stream = op.input("input", flow, RedisSource())
stream = op.map("raw dispatch", stream, RawDispatcher.handle_mq_message)
stream = op.filter("drop invalid raw", stream, lambda model: model is not None)
stream = op.map("clean dispatch", stream, CleaningDispatcher.dispatch_cleaner)
stream = op.filter("drop invalid clean", stream, lambda model: model is not None)
op.output(
