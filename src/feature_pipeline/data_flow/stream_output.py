...
def _to_qdrant_id(raw_id: str) -> str:
    # Qdrant accepts uint or UUID as point id; Mongo ObjectId strings are invalid.
...
class QdrantCleanedDataSink(StatelessSinkPartition):
    ...
    def write_batch(self, items: list[VectorDBDataModel]) -> None:
        ...
        raw_ids, data = zip(*payloads)
        ids = [_to_qdrant_id(raw_id) for raw_id in raw_ids]
...
class QdrantVectorDataSink(StatelessSinkPartition):
    ...
    def write_batch(self, items: list[VectorDBDataModel]) -> None:
        payloads = [item.to_payload() for item in items]
        raw_ids, vectors, meta_data = zip(*payloads)
        ids = [_to_qdrant_id(raw_id) for raw_id in raw_ids]
